...
in Editor. They are listed chronologically from top to bottom.
...
we click on `product` and a pane on the right should appear as in the screenshot below. The
...
![Product Detail](/img/tutorial/editor-load-detail.png)
...
since it is done instantly for us. The outcome is shown on the picture below.

![Recommendations Fetched](/img/tutorial/editor-getrecs.png)
...
![Code List Difference View - `codeSet` Entity Set Detail](/img/tutorial/editor-codelist-diff-entity-alone.png)
...
correspond to the URIs of [Publications Europa country code list](https://op.europa.eu/en/web/eu-vocabularies/dataset/-/resource?uri=http://publications.europa.eu/resource/dataset/country).

We investigate the changes further by clicking on `countries` in both
diagrams.

![Code list Difference View - `countries` Property Sets](/img/tutorial/editor-codelist-diff-property.png)
...
with well-known URIs such as [`country:USA`](https://publications.europa.eu/resource/authority/country/USA).
...
created `codeSet` entity set node, we get the view presented in the picture below. In the
...
![Applied Country Code List Recommendation](/img/tutorial/editor-codelist-after-view.png)

Adding a prefix can be done by clicking on the `Prefixes` button in the
top header and filling out the `Add Prefix` form to get the view as in the picture below.
...
![Country Code List Prefix Added](/img/tutorial/editor-add-prefix.png)
...
![Carbohydrates Nutrient Recommendation Difference View](/img/tutorial/editor-carbs-rec-entity.png)

The screenshot above shows the difference view for the topmost recommendation that suggests
...
contains type [`QuantitativeValueFloat`](http://purl.org/goodrelations/v1#QuantitativeValueFloat) from [Good Relations](https://www.heppnetz.de/projects/goodrelations/) vocabulary and that the literal for unit changed
from `"g"` to `"GRM"`.

When we click on the property `carbohydrates` that points to
`_carbohydrates`, we can see that its URI is set to
[`carbohydratesPer100g`](http://purl.org/foodontology#carbohydratesPer100g) from Food Ontology vocabulary
[food-purl-vocabulary-web](https://raw.githubusercontent.com/ailabitmo/food-ontology/master/food.owl). Clicking on literal property sets of
...
results in Editor showing a description view shown in the picture below.

![Carbohydrates Nutrient Recommendation Description](/img/tutorial/editor-carbs-rec-desc.png)

The description shown in the picture above provides of a brief explanation of the recommendation for Food Ontology
...
`food:Food` type. The model of Food Ontology is shown in the picture below.

![Food Ontology Model](/img/tutorial/foodontology.png)

We accept the recommendation and add prefixes for
...
is clicked on (picture below).

![Editor After Carbohydrate Recommendation with Prefixes](/img/tutorial/editor-carbs-after-wprefix.png)
...
does not look like in the picture below, then use the other nutrient recommendation
...
![Editor Energy Nutrient Recommendation Difference View](/img/tutorial/editor-nutr-rec-diff.png)
...
`nutriments` showing its detail (picture below).

![Editor after Nutrient Recommendations](/img/tutorial/editor-move-carbs.png)
...
![Move Carbohydrates Property Sets](/img/tutorial/editor-move-carbs-property.png)

The picture above shows how property sets and their properties are moved from one source
...
gives us the diagram shown in the picture below.

![Editor After Move Carbs and Energy to Food](/img/tutorial/editor-after-move-rm-nutr.png)
...
The end result is shown in the picture below. Adding a type to an entity set means that
...
in the corresponding property sets' inputs. The end result is shown in the picture below.

![Editor Add Schema ProductID](/img/tutorial/editor-add-sId.png)
...
ordering. We fill out the pattern according to how it is done in the picture below.
Below the pattern, we can see the preview of the entities' URIs. Then we click on `Ok` to set the
URIs.

![Set Food Product URIs](/img/tutorial/editor-set-food-uris.png)
...
set `gr:name`. We should be able to see the pane `Update Literals` (picture below).
We write `en` to the input for `Language` and click on `Ok` to set the
language tag `en` to all literals.

![Set Language Tag for Product Names](/img/tutorial/editor-set-lang.png)
...
input for the property set `countries` (picture below). We can scroll the pane to
check that language tags, URIs and the URI for `countries` are set
correctly.

![Set Countries Property](/img/tutorial/editor-set-countries.png)
